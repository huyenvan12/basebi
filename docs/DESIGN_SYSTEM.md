...
- **L3a · Sub Tabs** — nested navigation within a module (e.g. Team Shared's Shared Notes / Checklists / Monitor Log, or Delivery Tracker's Timeline / Calendar / Tasks). Same solid-fill language as L2, smaller size — because it's the same kind of action (navigation), it should look the same family. Always includes an icon per item: previously ambiguous (some L3a examples shipped with an icon, some didn't), now standardized so every L3a item gets one, matching L2's icon convention.
...
- **Breadcrumb** — for the rare case where a module has a genuinely 3-level-deep navigational hierarchy: app L2 > module's own L3a-equivalent > a further nested navigational switcher. The deepest level renders as plain text, not a pill: no background fill, active segment shown via `color:var(--accent)` + bold + underline, inactive segments `color:var(--text-dim)`, segments separated by "/" in a muted color (`var(--text-dim)`, reduced opacity). No icons on breadcrumb segments — kept deliberately minimal/compact. Example: "Templates / My Checklists / Shared with me". This tier exists specifically to avoid stacking a third visually-identical solid-fill pill-group under L2 and L3a, which reads as repetitive rather than hierarchical.

**Motivating case:** Team Shared's internal switcher (Shared Notes / Checklists / Monitor Log) is genuinely L3a-tier — nested inside the app's L2 "Team Shared" tab, not a second L2 — and should use L3a sizing, not the full L2 size it currently borrows. Checklists' own tri-tab (Templates / My Checklists / Shared with me), nested one level deeper still (inside Team Shared → Checklists), uses the new Breadcrumb tier instead of stacking a second L3a pill-group underneath it.
