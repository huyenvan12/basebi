...
Four tiers, distinguished by fill style (and, for the deepest tier, by having no fill at all) so navigation and filtering are never visually confused:
...
- **L3a · Sub Tabs** — nested navigation within a module (e.g. Team Shared's Shared Notes / Checklists / Monitor Log, or Delivery Tracker's Timeline / Calendar / Tasks). Same solid-fill language as L2, smaller size — because it's the same kind of action (navigation), it should look the same family. Always includes an icon per item: previously ambiguous (some L3a examples shipped with an icon, some didn't), now standardized so every L3a item gets one, matching L2's icon convention. **All L3a instances share one container class and one button class app-wide — no module gets its own separate container definition, even if the values happen to match another's.** This is an explicit rule, not just a convention: it's what prevents the shared recipe from drifting apart across modules over time, which is precisely what happened before this fix — Delivery Tracker's L3a container and Team Shared/Checklist's L3a container were separately defined and started as identical copies, but existed as two independent class definitions that could silently diverge. The pill-group container has no outer wrapper by default (no background/border/padding strip around it) — any module-specific spacing needed around it should be layout-only (margin), never a second fill layer, or the module ends up looking like it has two stacked containers instead of one.
...
- **Text Tabs** — for the rare case where a module has a genuinely 3-level-deep navigational structure: app L2 > module's own L3a-equivalent > a further nested switcher. The deepest level renders as plain inline text tabs, not a pill: no background fill, active item shown via `color:var(--accent)` + bold + underline, inactive items `color:var(--text-dim)`, items separated by a plain "·" character in a muted color (`var(--text-dim)`, reduced opacity) — not "/", which implies hierarchy/path. These items are sibling views at the same level, not a hierarchical path, so breadcrumb semantics and a path-style separator would be the wrong metaphor. No icons on Text Tab items — kept deliberately minimal/compact. Example: "Templates · My Checklists · Shared with me". This tier exists specifically to avoid stacking a third visually-identical solid-fill pill-group under L2 and L3a, which reads as repetitive rather than hierarchical.

**Motivating case:** Team Shared's internal switcher (Shared Notes / Checklists / Monitor Log) is genuinely L3a-tier — nested inside the app's L2 "Team Shared" tab, not a second L2 — and should use L3a sizing, not the full L2 size it currently borrows. Checklists' own tri-tab (Templates / My Checklists / Shared with me), nested one level deeper still (inside Team Shared → Checklists), uses the Text Tabs tier instead of stacking a second L3a pill-group underneath it.
